...
function shop_set(valID, kbn) {
  let data = shop_list.filter(function(item){
  ...
  switch (kbn) {
    case 'set':
      ...
      $('#thumbnail_name').val(data[0].photo);
      if (data[0].photo != '') {
        $('#thumbnail').attr("src", "https://s3-ap-northeast-1.amazonaws.com/rese-tak-ishiguro/public/img/shop_"+ data[0].id +"/"+data[0].photo);
        $('.thumbnail_content').css('display', 'block');
      } else {
        $('#thumbnail_name').val('');
        $('.thumbnail_content').css('display', 'none');
      };
